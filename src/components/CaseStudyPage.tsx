    ...
    <div className="case-page App" style={{ "--case-accent": study?.accent ?? "#fbbf24" } as React.CSSProperties}>
      ...
      {!study ? <main className="case-not-found"><h1 ref={heading} tabIndex={-1}>{ui("notFound")}</h1><a href="#projects">{ui("back")} <ArrowRight size={18} /></a></main> : (
        <main id="case-content" className="case-content">
          ...
          <section className="case-section">{sectionHeading("01", ui("scope"))}<div className="case-context-grid"><div><h3>{ui("challenge")}</h3><p>{text(study.challenge)}</p></div><div><h3>{ui("contribution")}</h3><ul className="case-contributions">{study.contributions.map((item, i) => <li key={i}><Check size={16} /><span>{text(item)}</span></li>)}</ul><span className="case-evidence">{study.evidence ? evidence(study.evidence) : ui(study.subprojects || study.gallery ? "repository" : "portfolio")}</span></div></div></section>

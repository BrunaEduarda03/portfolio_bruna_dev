...
export default function CaseSubprojects({ projects, clientName }: { projects: CaseSubproject[]; clientName: string }) {
  ...
  return <section className="case-subprojects" aria-labelledby="subprojects-title">
    <div className="case-section-heading"><span>{String(projects.length).padStart(2, "0")} / PROJECTS</span><h2 id="subprojects-title">{ui("subprojects")}</h2></div>
    <nav className="case-project-grid" aria-label={ui("subprojects")}>
      {projects.map((project, index) => <button type="button" key={project.id} onClick={() => jumpTo(project.id)}>
        <small>0{index + 1} / {clientName}</small><strong>{text(project.title)}</strong><p>{text(project.summary)}</p><span>{ui("explore")}<ArrowDown size={16} /></span>
      </button>)}
    </nav>
    {projects.map((project, index) => <article key={project.id} className="case-subproject" aria-labelledby={`${project.id}-title`}>
      <header><div><p className="case-eyebrow">{clientName} / 0{index + 1}</p><h2 id={project.id} tabIndex={-1}><span id={`${project.id}-title`}>{text(project.title)}</span></h2><p>{text(project.summary)}</p></div><a href={project.url} target="_blank" rel="noreferrer">{ui("visit")}<ArrowUpRight size={17} /></a></header>
